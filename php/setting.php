...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if($action == 'info'){
        ...
        if($_FILES['avatar']['size'] && $_FILES['avatar']['size'] < 301000){
            ...
            if($img_info){
                //
                $av_dir = DADA_ROOT.'avatar/large';
                if(!is_dir($av_dir)) {
                    mkdir($av_dir, 0777, true);
                    mkdir(DADA_ROOT.'avatar/normal', 0777, true);
                    mkdir(DADA_ROOT.'avatar/mini', 0777, true);
                }
                ...
                if(isset($img_obj)){
                    ...
                    if($max_px>73){
                        ...
                        imagejpeg($new_image, DADA_ROOT.'avatar/large/'.$cur_uid.'.png', 95);
                    }else{
                        imagejpeg($img_obj, DADA_ROOT.'avatar/large/'.$cur_uid.'.png', 95);
                    }
                    //normal
                    if($max_px>48){
                        $percent = 48/$max_px;
                        ...
                        ////目标文件，源文件，目标文件坐标，源文件坐标，目标文件宽高，源宽高
                        imagecopyresampled($new_image, $img_obj, 0, 0, 0, 0, $new_w, $new_h, $img_info[0], $img_info[1]);
                        imagejpeg($new_image, DADA_ROOT.'avatar/normal/'.$cur_uid.'.png', 95);
                    }else{
                        imagejpeg($img_obj, DADA_ROOT.'avatar/normal/'.$cur_uid.'.png', 95);
                    }
                    // mini
                    if($max_px>24){
                        $percent = 24/$max_px;
                        ...
                        ////目标文件，源文件，目标文件坐标，源文件坐标，目标文件宽高，源宽高
                        imagecopyresampled($new_image, $img_obj, 0, 0, 0, 0, $new_w, $new_h, $img_info[0], $img_info[1]);
                        imagejpeg($new_image, DADA_ROOT.'avatar/mini/'.$cur_uid.'.png', 95);
                    }else{
                        imagejpeg($img_obj, DADA_ROOT.'avatar/mini/'.$cur_uid.'.png', 95);
                    }
