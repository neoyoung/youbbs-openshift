    ...
    if($_FILES['filetoupload']['size']){
        ...
        if($pass){
            ...
            if($options['upyun_domain'] && $options['upyun_user'] && $options['upyun_pw']){
                ...
                $img_dir = 'http://'.$options['upyun_domain'].'.b0.upaiyun.com/'.$upload_dir;
            }else{
                $upload_dir = DADA_ROOT.'upload/'.$cur_uid;
                if(!is_dir($upload_dir)) {
                    mkdir($upload_dir, 0777, true);
                }
                $img_dir = 'http://'.$_SERVER['HTTP_HOST'].'/upload/'.$cur_uid;
            }
            ...
            if($is_img){
                ...
                if($options['upyun_domain'] && $options['upyun_user'] && $options['upyun_pw']){
                    ...
                    if($upyun->writeFile('/'.$upload_filename, $_FILES['filetoupload']['tmp_name'], true)){
                        $rsp['status'] = 200;
                        $rsp['url'] = '附件：'.$up_name.' '.$img_dir.'/'.$new_name;
                        $rsp['msg'] = '附件已成功上传';
                    }else{
